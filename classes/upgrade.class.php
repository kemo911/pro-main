...
class PHPLogin_upgrade extends Generic {
	...
	function __construct() {
		...
		// Begin !
		if ( isset( $_SESSION['jigowatt']['user_level'] ) && in_array( 1, $_SESSION['jigowatt']['user_level'] )) {
	 ...
	 */
	public function jigowatt_upgrade() {
...
// 		</div>';
		...
		if (!empty($_GET['update'])) :
			...
			if ( $phplogin_db_version < 1503250 )
				$this->upgrade_401();

			if ( $phplogin_db_version < 1706120 )
				$this->upgrade_410();
...
//			$this->result = "<div class='alert alert-success'>Your database has been successfully updated !</div>";
	...
	private function upgrade_250() {
		...
		parent::query("
			...
			('email-activate-subj', 'You''ve activated your account !'),
			...
			('email-welcome-subj', 'Thanks for signing up :)'),
			...
			('email-add-user-subj', 'You''re registered !'),
	...
	private function upgrade_401() {

		// Removing level_level dependency
		$sql = "ALTER TABLE  `login_levels`DROP COLUMN `level_level`";
		parent::query($sql);

		parent::updateOption('phplogin_db_version', 1303250);

	}

	private function upgrade_410() {

		parent::query('
			ALTER TABLE `login_users`
				ADD `phone` VARCHAR( 12 ) NULL DEFAULT NULL,
				ADD `tmp_auth_token` INT( 8 ) NULL DEFAULT NULL,
				ADD `use_two_factor_auth` VARCHAR( 2 ) NULL DEFAULT NULL,
				ADD `sms_time` INT( 11 ) NULL DEFAULT NULL;
		');

		parent::query('
			ALTER TABLE `login_confirm` CHANGE `data` `data` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL;
			ALTER TABLE `login_confirm` CHANGE `username` `username` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL;'
		);

		parent::updateOption('phplogin_db_version', 1706120);
