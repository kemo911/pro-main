	...
	private function register() {
		...
		if(empty($this->error)) {
			...
			$params = array(
				...
				':name'       => preg_replace('/[^A-Za-z0-9\-]/', '', $this->settings['name']),
				':email'      => $this->settings['email'],
				':username'   => preg_replace('/[^A-Za-z0-9\-]/', '', $this->settings['username']),
			...
			if ( $requireActivate ) :
				...
				$sql = sprintf("INSERT INTO `login_confirm` (`username`, `key`, `email`, `type`)
								VALUES ('%s', '%s', '%s', '%s');",
					preg_replace('/[^A-Za-z0-9\-]/', '', $this->settings[$this->username_type]), $key, $this->settings['email'], 'new_user');
				parent::query($sql);
			endif;
