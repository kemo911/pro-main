...
class Generic extends Connect {
	...
	public function definePaths() {
		...
		if (!defined('phplogin_db_version'))    define( 'phplogin_db_version', '1706120');
		if (!defined('phplogin_version'))       define( 'phplogin_version',    '4.1.0');

	}

	/**
	 * Hashes a password for either BCRYPT, MD5 or SHA256.
	 *
	 * If hashing SHA256, a unique salt will be hashed with it.
	 *
	 * @param     string        $password    A plain-text password.
	 * @return    string|bool   Hashed password or false if not a known method.
	 */
	public function hashPassword( $password ) {

		$type = $this->getOption('pw-encryption');

		// Checks if the pw should be MD5
		if ( $type == 'MD5' ) {
		    return md5( $password );
        }

		// Checks if the pw should be SHA256
		if ( $type == 'SHA256' ) {
		...
		// Checks if the pw should be BCRYPT (default recommended since PHP 5.5)
		if ( $type == 'BCRYPT' ) {
